...
const int INPUT_DIM = ROLE_FEATURES * 2;
...
#ifdef _WIN32
static string wide_to_utf8(const wchar_t *ws)
{
...
static string direct_arg_utf8(int argc, char **argv)
{
    int wargc = 0;
    LPWSTR *wargv = CommandLineToArgvW(GetCommandLineW(), &wargc);
    if (wargv != nullptr && wargc == 2) {
        string out = wide_to_utf8(wargv[1]);
        LocalFree(wargv);
        return out;
    }
    if (wargv != nullptr) LocalFree(wargv);
    return argc >= 2 ? string(argv[1]) : string();
}
#else
static string direct_arg_utf8(int argc, char **argv)
{
...
u8_t idx[16];
Name name_initial;
thread_local char s_tmp[256];
thread_local unordered_map<string, Name> build_cache;
const size_t BUILD_CACHE_LIMIT = 200000;
...
// cerr<<"ok1_3\n";
    ...
    if (build_cache.size() >= BUILD_CACHE_LIMIT) build_cache.clear();
...
int prop[8];
thread_local char NAME_ALL[N*2];
thread_local unsigned int NAME_GUARD = 0xDEADBEEF;
int FREQ[N];
void cvt_name()
{
...
static bool is_number_token(const string &s)
{
...
static string normalize_duo_input_line(const string &line)
{
    ...
    getline(iss, rest);
    rest = trim_copy(rest);
    if (is_number_token(first) && rest.find('+') != string::npos && rest.find('@') != string::npos) {
        return rest;
    }
    return s;
}
...
struct InferenceStats
{
    float y_scale = 1.0f;
    float y_bias = 0.0f;
    float x_mean[INPUT_DIM]{};
    float x_std[INPUT_DIM]{};

    bool load_from_text(const string &path)
    {
        ifstream fin(path);
        ...
        if (vals.size() < 2 + INPUT_DIM * 2) return false;
        y_scale = vals[0];
        y_bias = vals[1];
        for (int i = 0; i < INPUT_DIM; i++) x_mean[i] = vals[2 + i];
        for (int i = 0; i < INPUT_DIM; i++) {
            float s = vals[2 + INPUT_DIM + i];
            x_std[i] = (fabs(s) < 1e-12f ? 1.0f : s);
        }
        return true;
    }
};

static bool extract_feature(const string &duo, vector<float> &feat)
{
    feat.assign(INPUT_DIM, 0.0f);
    int ll = 0, rr = 0, p = -1;
    if (!split_duo_line(duo, ll, p, rr)) return false;

    bool same_clan = clan_key(NAME_ALL, ll, p - 1) == clan_key(NAME_ALL, p + 1, rr);
    Name x = build(NAME_ALL, ll, p - 1);
    Name y = build(NAME_ALL, p + 1, rr);
    ...
    for (int i = 0; i < ROLE_FEATURES; i++) feat[i] = (float)X.x[i];
    for (int i = 0; i < ROLE_FEATURES; i++) feat[i + ROLE_FEATURES] = (float)Y.x[i];
    return true;
}

static void normalize_feature(vector<float> &feat, const InferenceStats &stats)
{
    for (int i = 0; i < INPUT_DIM; i++) {
        feat[i] = (feat[i] - stats.x_mean[i]) / stats.x_std[i];
    }
}
...
{
public:
    explicit Predictor(const string &model_path)
        : env(ORT_LOGGING_LEVEL_WARNING, "predict_mech")
    {
...
#ifdef _WIN32
        wstring wmodel(model_path.begin(), model_path.end());
        session = make_unique<Ort::Session>(env, wmodel.c_str(), session_options);
#else
    ...
    float predict(vector<float> &input_vec, const InferenceStats &stats)
    {
        vector<int64_t> shape = {1, INPUT_DIM};
        ...
        return pred[0] * stats.y_scale + stats.y_bias;
    }

    vector<float> predict_batch(vector<float> &input_vec, int batch_size, const InferenceStats &stats)
    {
        if (batch_size <= 0) return {};
        vector<int64_t> shape = {batch_size, INPUT_DIM};
        Ort::MemoryInfo memory_info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
        Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
            memory_info,
            input_vec.data(),
        ...
        float *pred = output_tensors[0].GetTensorMutableData<float>();
        vector<float> out(batch_size);
        for (int i = 0; i < batch_size; i++) {
            out[i] = pred[i] * stats.y_scale + stats.y_bias;
        }
        return out;
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc == 2) {
        string duo = normalize_duo_input_line(direct_arg_utf8(argc, argv));
        if (duo.find('+') != string::npos && duo.find('@') != string::npos) {
            InferenceStats stats;
            if (!stats.load_from_text("scale.txt") && !stats.load_from_text("model4_scale.txt")) {
                cerr << "[ERROR] failed to load default scale file: scale.txt or model4_scale.txt\n";
                return 1;
            }
            for (int i = 0; i < 35; i++) Map[skillNameMap_2[i]] = i;

            vector<float> feat(INPUT_DIM);
            if (!extract_feature(duo, feat)) {
                cerr << "[ERROR] invalid duo format\n";
                return 1;
            }
            normalize_feature(feat, stats);

            Predictor predictor("model4.onnx");
            float score = predictor.predict(feat, stats);
            printf("%.2f\n", score);
            return 0;
        }
    }
    ...
    if (argc < 5) {
        cerr << "Usage: predict_mech.exe model4.onnx input_duos.txt model4_scale.txt thread_count [min_score] [output.txt]\n";
        cerr << "Single duo shortcut: predict_mech.exe \"name1@team+name2@team\" (uses model4.onnx and scale.txt in current directory)\n";
        cerr << "Input accepts one duo per line, optional first-line count, or old 'score duo' lines.\n";
        return 1;
    }

    string model_path = argv[1];
    string input_path = argv[2];
    string scale_path = argv[3];
    ...
    if (argc >= 6) {
        string arg5 = argv[5];
        if (is_number_token(arg5)) {
            min_score = (float)atof(arg5.c_str());
        } else {
            output_path = arg5;
        }
    }
    if (argc >= 7) {
        string arg6 = argv[6];
        if (is_number_token(arg6)) {
            min_score = (float)atof(arg6.c_str());
        } else {
            output_path = arg6;
        }
    }
    ...
    InferenceStats stats;
    if (!stats.load_from_text(scale_path)) {
        cerr << "[ERROR] failed to load scale file: " << scale_path << '\n';
        return 1;
    }
    fprintf(stderr, "[INFO] loaded scale\n");
    fflush(stderr);

    ifstream fin(input_path, ios::in | ios::binary);
    if (!fin.is_open()) {
        cerr << "[ERROR] failed to open input: " << input_path << '\n';
        return 1;
    }

    ofstream fout(output_path, ios::out | ios::binary);
    if (!fout.is_open()) {
        cerr << "[ERROR] failed to open output: " << output_path << '\n';
        return 1;
    }
    fprintf(stderr, "[INFO] opened files\n");
    fflush(stderr);

    for (int i = 0; i < 35; i++) Map[skillNameMap_2[i]] = i;

    constexpr size_t BATCH_SIZE = 1024;
    constexpr size_t MAX_QUEUE_BATCHES = 64;
    deque<vector<string>> tasks;
    mutex queue_mutex;
    mutex output_mutex;
    ...
    atomic<long long> enqueued_count(0);
    atomic<long long> processed(0);
    atomic<long long> kept(0);
    atomic<long long> skipped(0);

    cerr << "[INFO] thread_count=" << thread_count
         << " output=" << output_path
         << '\n';
    ...
    for (int t = 0; t < thread_count; t++) {
        workers.emplace_back([&, t]() {
            try {
                ...
                if (t == 0) {
                    lock_guard<mutex> lk(progress_mutex);
                    cerr << "[INFO] loaded onnx model in workers\n";
                }
                vector<float> feat(INPUT_DIM);
                vector<float> batch_input;
                vector<string> batch_duos;
                string local_out;
                local_out.reserve(1 << 20);
                batch_input.reserve(BATCH_SIZE * INPUT_DIM);
                batch_duos.reserve(BATCH_SIZE);

                auto flush_local = [&]() {
                    if (local_out.empty()) return;
                    lock_guard<mutex> out_lock(output_mutex);
                    fout.write(local_out.data(), (streamsize)local_out.size());
                    if (!fout.good()) {
                ...
                while (true) {
                    vector<string> batch;
                    {
                        ...
                        cv_not_empty.wait(lk, [&]() {
                            return input_done || fatal_error.load() || !tasks.empty();
                        });
                        if (fatal_error.load() || (input_done && tasks.empty())) break;
                    ...
                    batch_input.clear();
                    batch_duos.clear();
                    for (string &raw_line : batch) {
                        if (!raw_line.empty() && raw_line.back() == '\r') raw_line.pop_back();
                        string duo = normalize_duo_input_line(raw_line);
                        if (duo.empty()) continue;
                        if (duo.find('+') == string::npos || duo.find('@') == string::npos) {
                            skipped++;
                            continue;
                        }
                        ...
                        normalize_feature(feat, stats);
                        batch_input.insert(batch_input.end(), feat.begin(), feat.end());
                        batch_duos.push_back(std::move(duo));
                    }
                    ...
                    vector<float> scores = predictor.predict_batch(
                        batch_input,
                        (int)batch_duos.size(),
                        stats
                    );
                    long long cur_processed = processed.fetch_add((long long)batch_duos.size()) + (long long)batch_duos.size();

                    for (size_t i = 0; i < batch_duos.size(); i++) {
                        float score = scores[i];
                        if (score >= min_score) {
                            char buf[64];
                            int len = snprintf(buf, sizeof(buf), "%.2f ", score);
                            local_out.append(buf, len);
                            local_out += batch_duos[i];
                            local_out.push_back('\n');
                            kept++;
                            if (local_out.size() >= (1u << 20)) {
                                flush_local();
                                if (fatal_error.load()) break;
                            }
                        }
                    ...
                    if (cur_processed / 10000 != (cur_processed - (long long)batch_duos.size()) / 10000) {
                        lock_guard<mutex> lk(progress_mutex);
                        cerr << "processed=" << cur_processed
                             << " kept=" << kept.load()
                ...
                lock_guard<mutex> lk(progress_mutex);
                cerr << "[ERROR] worker " << t << " failed: " << e.what() << '\n';
                fatal_error.store(true);
                cv_not_empty.notify_all();
                cv_not_full.notify_all();
            } catch (...) {
                lock_guard<mutex> lk(progress_mutex);
                cerr << "[ERROR] worker " << t << " failed with unknown exception\n";
                fatal_error.store(true);
                cv_not_empty.notify_all();
                cv_not_full.notify_all();
            }
    ...
    while (!fatal_error.load() && getline(fin, line)) {
        read_count++;
        string normalized = normalize_duo_input_line(line);
        if (normalized.empty()) continue;
        if (first_content_line && is_number_token(normalized) && normalized.find('+') == string::npos) {
            first_content_line = false;
            continue;
        }
        first_content_line = false;

        current_batch.push_back(std::move(line));
        if (current_batch.size() >= BATCH_SIZE) {
            unique_lock<mutex> lk(queue_mutex);
            cv_not_full.wait(lk, [&]() {
                return fatal_error.load() || tasks.size() < MAX_QUEUE_BATCHES;
            });
            if (fatal_error.load()) break;
            enqueued_count += (long long)current_batch.size();
            tasks.push_back(std::move(current_batch));
            current_batch = vector<string>();
            current_batch.reserve(BATCH_SIZE);
            lk.unlock();
    ...
    if (!fatal_error.load() && !current_batch.empty()) {
        unique_lock<mutex> lk(queue_mutex);
        cv_not_full.wait(lk, [&]() {
            return fatal_error.load() || tasks.size() < MAX_QUEUE_BATCHES;
        });
    ...
    cerr << "[INFO] read=" << read_count.load()
         << " enqueued=" << enqueued_count.load()
         << " processed=" << processed.load()
         << " kept=" << kept.load()
         << " skipped=" << skipped.load()
         << " fatal=" << (fatal_error.load() ? 1 : 0)
         << '\n';
    if (fatal_error.load()) return 2;
    return 0;
}
/*
